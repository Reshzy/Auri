...
type ManifestShape = {
  ...
  orientation?: string;
  maxDayCount?: number;
  leftCellMap?: Record<string, unknown>;
  rightCellMap?: Record<string, unknown>;
...
async function activateTemplateVersions(input: {
  ...
  const existingRows = await db
    ...
    .where(
      and(
        eq(templateVersions.templateKey, DTR_TEMPLATE_KEY),
  ...
  const manifestJson = {
    ...
    maxDayCount: input.manifest.maxDayCount ?? 31,
    leftCellMap: input.manifest.leftCellMap,
    rightCellMap: input.manifest.rightCellMap,
    requiredFormulas: input.manifest.requiredFormulas,
  };

  await db.transaction(async (tx) => {
    // Deactivate only DTR rows — do not touch accomplishment.
    await tx
      .update(templateVersions)
      .set({ isActive: false, updatedAt: new Date().toISOString() })
      .where(eq(templateVersions.templateKey, DTR_TEMPLATE_KEY));
...
async function main() {
  ...
  if (!existsSync(runtimePath) || !existsSync(manifestPath)) {
    throw new Error("Runtime template/manifest missing. Run pnpm xlsx:prepare first.");
  }
  ...
  if (manifest.runtimeSha256 !== runtimeHash) {
    throw new Error(
      "Local runtime hash does not match manifest. Re-run pnpm xlsx:prepare.",
    );
  }
  ...
  if (!url || !serviceKey) {
    console.log(
      "templates:upload:xlsx — Storage env missing; attempting local DB activation only.",
    );
    ...
    const storagePath = `local/${DTR_TEMPLATE_KEY}/v${version}/${DTR_RUNTIME_FILE}`;
  ...
  const storagePath = `${DTR_TEMPLATE_KEY}/v${version}/${DTR_RUNTIME_FILE}`;
  ...
  console.log("templates:upload:xlsx OK");
  console.log(`storagePath=${storagePath}`);
  console.log(`sha256=${runtimeHash}`);
  console.log("template_versions: active dtr v1 reconciled (accomplishment untouched)");
  process.exit(0);
}

main().catch((error) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`templates:upload:xlsx FAILED: ${message}`);
  process.exit(1);
});
